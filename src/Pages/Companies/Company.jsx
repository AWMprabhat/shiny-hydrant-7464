...
const Company = () => {
  ...
  return (
    <>
      ...
      {company.length > 0 &&
        company.map((t) => (
          <Flex key={t.id} border="1px solid red" direction="column">
            <Box>
              <Image w="200px" m="auto" h="150px" src={t.image} alt={t.name} />
            </Box>
            <Box>{t.name} </Box>

            <Box>
              <Box display="flex" m="auto" w="60%">
                <Box display="flex" alignItems={'center'} w="50%" justifyContent="right" pr="0.3rem" borderRight="1px solid grey"><AiFillStar color="gold" /> {t.rating}</Box>
